...
static Symbols globalVariables;
...
static void topLevelDeclaration()
{
	...
	if (lookahead == '[') {
	    match('[');
	    globalVariables.push_back(declareVariable(name, Type(typespec, indirection, number())));
	    match(']');
	    ...
	    if (lookahead == ')') {
		match(')');
		symbol = declareFunction(name, Type(typespec, indirection, nullptr));
	    ...
	    globalVariables.push_back(declareVariable(name, Type(typespec, indirection)));
...
int main()
{
	...
	globalsGenerator(globalVariables);
    closeScope();
    exit(EXIT_SUCCESS);
}
